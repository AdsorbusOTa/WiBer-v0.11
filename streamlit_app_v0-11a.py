# -*- coding: utf-8 -*-
VERSION = "v0-11a"
RELEASE_DATE = "15.04.2025"  # manuell gepflegt
...
if plz:
    ...
    if coords:
        ...
        selected_year = st.number_input("📅 Jahr auswählen", min_value=2019, max_value=date.today().year - 1, value=2023)
        auto = st.toggle("🔄 Jahr automatisch ermitteln", value=False)
        
        # Bei Betätigung Button, werden die Daten abgerufen in nach SessionState übertragen:
        if st.button("🧪 Starte Stationssuche"):
        ...
        if daten:
            ...
            selected_index = st.selectbox(t["weather_select_station"], range(len(station_ids)), format_func=lambda i: station_labels[i])
            station_id = station_ids[selected_index]

            # 🌡️ Temperaturgrenzen
            st.markdown(t["auto_key_1"])
            ...
            with st.expander(f"{t['weather_expander_title']} {jahr}", expanded=False):
                with st.spinner("Stationen & Parameter werden geladen..."):
                    try:
                        ...
                        # info = daten[station_id]
                        # st.success(f"Daten von Station {station_id} – {info['name']} – Entfernung: {round(info['distance'], 1)} km")
                        zählung = {
                        ...
                        chart = alt.Chart(df_sorted).mark_area(
                            ...
                            tooltip=["Stunde", "value", "Temperaturbereich"]
                        ).properties(
                            width=700,
                            height=400,
                            title="📈 Temperaturverlauf (Jahresdauerlinie)"
                        )

                        # 🎨 Altair-Liniendiagramm
                        # chart = alt.Chart(df_sorted).mark_line().encode(
                        #     x=alt.X("Stunde:Q", title="Stunde im Jahr [h]"),
                        #     y=alt.Y("value:Q", title="Temperatur [°C]"),
                        #     color=alt.Color("Temperaturbereich:N", title="Temperaturbereich"),
                        #     tooltip=["Stunde", "value", "Temperaturbereich"]
                        # ).properties(
                        #     width=700,
                        #     height=400,
                        #     title="📈 Temperaturverlauf (sortiert)"
                        # )
...
if df_kennfeld is not None:
    ...
    hwt_input = st.number_input(t["design_input_ht"], min_value=40.0, max_value=100.0, value=85.0, step=1.0)
    rkt_input = st.number_input(t["design_input_mt"], min_value=15.0, max_value=45.0, value=27.0, step=1.0)
    kwt_out_input = st.number_input(t["design_input_nt"], min_value=5.0, max_value=20.0, value=15.0, step=0.5)
